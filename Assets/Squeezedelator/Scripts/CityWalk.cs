...
public class CityWalk : MonoBehaviour //don't forget to change the script name if you haven't
{
    ...
    void Update()
    {
        ...
        {
            Vector3 point;
            if (RandomPoint(centrePoint.position, _range, out point)) //pass in our centre point and radius of area
            {
                Debug.DrawRay(point, Vector3.up, UnityEngine.Color.blue, 1.0f); //so you can see with gizmos
                StartCoroutine(WaitRandomly(point));              
            }
        }
    }
